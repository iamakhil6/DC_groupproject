...
class SI39:
    ...
    def funcSocketListening(self,):
        ...
        while True:
            ...
            data = con.recv(1024)
            self.processCount += 1
#            p = Process(target=self.mathTask, args=(1,))
#            p.start()
...
#            with open('SImetrics.csv', 'a') as a:
#                csvwriter = csv.writer(a, delimiter=',')
#                csvwriter.writerow([result[-3] , +str(c[minindex]), result[-1]])
...
#            num = self.fibonacci(senderProcessID)
            ...
            s.sendall(b'Result for process ID_'+bytes(senderProcessID, 'utf-8')+b'_is_'+ bytes(str(num), 'utf-8'))
            s.close()
...
if __name__ == '__main__':
    ...
    PPFLAG = False
    if False:
        with open('transfermetrics.csv', 'w') as w:
            csvwriter = csv.writer(w, delimiter=',')
            csvwriter.writerow(['No of Processes', 'Process ID', 'Transferred System', 'Time Taken'])
        with open('executionmetrics.csv', 'w') as w:
            csvwriter = csv.writer(w, delimiter=',')
            csvwriter.writerow(['Process Count', 'Time Taken', 'Message Count'])
...
#                print('data received from server ', key, data, int.from_bytes(data, 'big'))
                obj.messageOverhead += len(serverDetails.keys())
#                print("Msg over load verification ", obj.messageOverhead)
            ...
            if int.from_bytes(min(b), 'big') < threshold:
                ...
                if TEXTFLAG:
                    ...
                    job_server = pp.Server(0, ppservers=(serverDetails[c[minindex]][0] + ':' + str(serverDetails[c[minindex]][3]),))
                    result = job_server.submit(obj.mathTask, (dataStack[-1],))
...
#                    print('Sum of lsit ', result())
                    ...
                    socketCon = socket(AF_INET, SOCK_STREAM)
                    socketCon.connect((serverDetails[c[minindex]][0], serverDetails[c[minindex]][2]))
#                    print('Data passing ', dataStack[-1], obj.resultDict)
